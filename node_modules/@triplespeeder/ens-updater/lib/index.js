...
module.exports = function Updater() {
    ...
    // module variables
    let resolver, registry, registrar, reverseRegistrar, controllerAddress, verbose, node, web3, ensName, dryrun, estimateGas, gasPrice, gas, needsMigration
    ...
    function verifyMigrationStatus() {
        if (needsMigration) {
            throw Error(`${ensName} can not be changed until it is migrated to the current registry`)
        }
    }
    ...
    async function getRegistrar(ensName) {
        verbose && console.log(`Getting Registrar...`)
    ...
    async function getRegistrant() {
        ...
        if (labels.length === 2) {
            ...
            try {
                ...
                verbose && console.log(`Failed to get registrant for ${ensName}. Error: ${error}`)
                return undefined
            }
        } else {
            return undefined
    ...
    async function setup(options) {
        ...
        if (ensName) {
            verbose && console.log(`Getting Registry and Resolver...`)
            ...
            if (!await registry.recordExists(node)) {
                ...
                verbose && console.log(`\t${ensName} not found in current registry. Checking old registry at ${oldRegistryAddress}`)
    ...
    async function getInfo() {
        ...
        return {
            ...
            Registrar: registrar ? registrar.address : zeroAddress
        }
    }
